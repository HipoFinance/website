...
- **Coral has two tokens.** `--color-accent` is coral as a _foreground_ (links, the hero highlight, the APY stat) and darkens to `#e0574b` on cream so it keeps its contrast. `--color-accent-fill` is coral as a _solid fill_ under `text-on-accent` (buttons, the banner, `::selection`) and is the brand `#ff7e73` in both schemes. Using `bg-accent` is a bug — the utility does not exist.
