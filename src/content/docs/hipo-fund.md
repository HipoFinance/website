---
title: 'Hipo Fund'
description: 'The Hipo Fund — an on-chain treasury backing the HPO token.'
---

<figure><img src="/docs/images/hipo-fund-1.jpg" alt=""><figcaption></figcaption></figure>

### 📌 Overview

The **Hipo Fund** is a transparent, DeFi-native investment treasury created to support the long-term sustainability of the Hipo ecosystem and the HPO token. It reinvests proceeds from HPO token sales into TON on-chain assets to **diversify risk**, **generate yield**, and **strengthen the Hipo economy** by backing the value of HPO.

---

:::note
Hipo Fund Wallet Upgraded to Multisig for Enhanced Security
:::

### 💰 Fund Summary — opening report, April 18, 2025

- **Initial Capital:** $186,963.96
- **Start Reporting Date:** April 18, 2025
- **Fund Wallet Address:** [hipofund.ton](https://tonviewer.com/EQDa2GcC9KwiWIL6jmrGp2ulhC7hnNo8DUunEtkMKe4r_Dnr) — `EQDa2GcC9KwiWIL6jmrGp2ulhC7hnNo8DUunEtkMKe4r_Dnr` (multisig)
- **Previous wallet (before the multisig migration):** [UQBwGlrpvnLzWM1qOXW2DPe99mg1W5pcf2R_uxSeDiVDdLfG](https://tonviewer.com/UQBwGlrpvnLzWM1qOXW2DPe99mg1W5pcf2R_uxSeDiVDdLfG)

---

#### 🔸 Initial Portfolio Allocation

| Asset              | Amount       | Allocation | Value (USD) | Notes                             |
| ------------------ | ------------ | ---------- | ----------- | --------------------------------- |
| Stablecoins (USDT) | 3,304.14     | 1.76%      | $3,304.14   | Capital preservation & dry powder |
| hGRAM              | 34,955.22    | 59.28%     | $111,405.91 | Staked GRAM from Hipo             |
| HPO                | 6,754,307.59 | 38.95%     | $15,000     | Governance & profit-sharing token |
| Other (GRAM)       | 5.30         | 0.01%      | $15.87      | Direct GRAM exposure              |

> - Values are as reported on April 18, 2025.
> - All assets are held on-chain and can be verified via [hipofund.ton](https://tonviewer.com/EQDa2GcC9KwiWIL6jmrGp2ulhC7hnNo8DUunEtkMKe4r_Dnr)
> - Hipo Fund does not include any allocation from HPO tokenomics. All HPO held by the fund has been purchased directly from the market.

---

### 🔒 Management Principles

- **Fully On-chain & Transparent**\
  All fund assets are visible and trackable on TON. We only invest in assets that can be transparently monitored.
- **Quarterly Reporting**\
  We will publish a full, transparent report every season detailing performance, changes, and current holdings.
- **Risk-Controlled Growth**\
  We focus on long-term capital protection, stable growth, and low-risk yield generation.
- **Governance-Driven**\
  Over time, HPO holders will guide fund direction through DAO voting. Professional treasury management will evolve alongside community participation.

---

### 💜 For the Hipo Community

The Hipo Fund is owned by the community. Its growth supports the value and sustainability of the HPO and supports every HPO holder. We are committed to **radical transparency**, **open governance**, and **DeFi-native best practices**.

Want to suggest strategies, DeFi tools, or TON projects for the fund? Join the conversation in [@hipo_chat on Telegram](https://t.me/hipo_chat).
