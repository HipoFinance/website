...
- A multisig can now unstake part of its hGRAM: the app builds the order and
  shows the three values to copy into its own UI.
- The guidance dialog freezes its amount, rate option and query id when it
  opens, so a copied payload cannot drift.
- The destination is named as the holder's own hGRAM wallet contract, with a
  Tonviewer link to verify it.
- The docs and FAQ no longer say a partial unstake is impossible, in English and
  all nine released locales.
- A rejected clipboard write in the dialog's copy fields is handled.

## 2026-09-03 — [detailed report](changelog/2026-09-03-brand-kit-rebuild.md)

- The brand kit ZIP is now generated by `npm run brand-kit` from the live marks
  in `public/` plus the two lockups in the new `brand/`.
- hGRAM added (flat and 3D), HPO gained a derived flat coin, hTON moved to a
  `hTON (legacy)` folder.
- Every logo now ships in a light-background colorway, and the logotype ships on
